...
def Video(video_path):
    ...
    for i, image in enumerate(frame_indices):
        ...
        # print(image_path)
        target_image_path = os.path.join(opt.save_path, class_name, vid, 'image_%05d.jpg'%i)
        if not os.path.exists(os.path.join(opt.save_path, class_name, vid)):
            os.makedirs(os.path.join(opt.save_path, class_name, vid))
...
if __name__ == '__main__':
    ...
    # base_dir = "/userhome/dataset/VATEX/original_trainval_frames/**/*"
    # videos_dir = glob.glob(base_dir)
    #
    opt.save_path = "/userhome/dataset/VATEX/28frame-vatex/"
    ...
    # pool.close()
    # pool.join()
    path = "/userhome/dataset/VATEX/original_trainval_frames/fixing hair/TLzXbcs9Yvc_000028_000038"
    Video(path)
